// Heavily inspired by: https://github.com/kubernetes/client-go/blob/940f07561929308fc33fc58bfa185f0544b84846/util/homedir/homedir.go#L31
// only hardcoded config dir was changed.
package homedir
...
// On Windows:
// 1. the first of %HOME%, %HOMEDRIVE%%HOMEPATH%, %USERPROFILE% containing a `.codeowners-validator\config` file is returned.
...
func HomeDir() string {
	if runtime.GOOS == "windows" {
		...
		for _, p := range []string{home, homeDriveHomePath, userProfile} {
			...
			if _, err := os.Stat(filepath.Join(p, ".codeowners-validator", "config")); err != nil {
				continue
			}
			return p
		}
